...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    ...
    <?php if( get_theme_mod( 'vcard_cv_resume_single_postdate',true) == 1 || get_theme_mod( 'vcard_cv_resume_single_author',true) == 1 || get_theme_mod( 'vcard_cv_resume_single_comments',true) == 1 || get_theme_mod( 'vcard_cv_resume_single_time',true) == 1) { ?>
        <div class="post-info p-2 my-3">
            ...
            <?php if(get_theme_mod('vcard_cv_resume_single_author',true)==1){ ?>
                <span><?php echo esc_html(get_theme_mod('vcard_cv_resume_single_post_meta_field_separator', '|'));?></span> <i class="fas fa-user me-2"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
            <?php } ?>

            <?php if(get_theme_mod('vcard_cv_resume_single_comments',true)==1){ ?>
                <span><?php echo esc_html(get_theme_mod('vcard_cv_resume_single_post_meta_field_separator', '|'));?></span> <i class="fa fa-comments me-2" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'vcard-cv-resume'), __('0 Comments', 'vcard-cv-resume'), __('% Comments', 'vcard-cv-resume') ); ?></span>
            <?php } ?>

            <?php if(get_theme_mod('vcard_cv_resume_single_time',true)==1){ ?>
              <span><?php echo esc_html(get_theme_mod('vcard_cv_resume_single_post_meta_field_separator', '|'));?></span> <i class="fas fa-clock"></i> <span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
            <?php } ?>
        </div>
    <?php } ?>
    ...
    <?php if(get_theme_mod('vcard_cv_resume_toggle_tags',true)==1){ ?>
    ...
    <?php } ?>
        ...
        if ( is_singular( 'attachment' ) ) {
            ...
            the_post_navigation( array(
                'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next', 'vcard-cv-resume' ) . '</span> ' .
                    '<span class="screen-reader-text">' . __( 'Next post:', 'vcard-cv-resume' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
                'prev_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Previous', 'vcard-cv-resume' ) . '</span> ' .
