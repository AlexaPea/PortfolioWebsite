...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('segovia_gmap_function')) {
  function segovia_gmap_function( $atts, $content = NULL ) {
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = SEGOVIA_PLUGIN_ASTS . '/images/map-marker.png';
    }
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.segva-gmap-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. segovia_core_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }

    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' segva-gmap-'. $e_uniqid;
    ...
    // Map Tab ID
    if ($gmap_id) {
      $gmap_open = '<div '. $gmap_id .' class="segva-gmap-tab tab-pane fade">';
      $gmap_close = '</div>';
    } else {
      $gmap_open = '';
      $gmap_close = '';
    ...
    echo $gmap_open;
    echo '<div id="map-'. $e_uniqid .'" class="segva-google-map '. $custom_css . $class . $styled_class .'"></div>';
    echo $gmap_close;
...
add_shortcode( 'segva_gmap', 'segovia_gmap_function' );
